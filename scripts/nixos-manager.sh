...
# Este script mora em scripts/ — a raiz do flake é um nível acima.
NIXOS_DIR="${NIXOS_DIR:-$(cd "$SCRIPT_DIR/.." && pwd)}"
...
# set from the `hostname` specialArg in system/profiles/base.nix) — this
...
# (legacy path) or run_nh_os (preferred path, see below).

# Extra flags for the underlying nix build, per flake attr.
...
#
# Kept only as: (a) the legacy/non-flake path (build_legacy), (b) the
# rollback path (nh has no rollback subcommand), and (c) an automatic
# fallback inside run_nh_os for hosts where `nh` isn't installed yet.
run_nixos_rebuild() {
  local action="$1"
  shift
  local -a extra_flags=()
...
# run_nh_os <action> [extra nh args...]
# action: switch | boot | test | build
#
# Preferred path for every flake-based rebuild (build_flake, build_flake_dry,
# update_system, and the post-clean boot sync). Wraps `nh os <action> <dir>
# -H <FLAKE_ATTR>` instead of `nixos-rebuild ... --flake <dir>#<attr>`:
#   - nh self-elevates with sudo internally — never prefix this with sudo.
#   - Dell's OOM-safe flags (--max-jobs 1 --cores 1) go after `--`, since nh
#     forwards anything past `--` to the underlying `nix build`.
#   - Same before/after generation verification as run_nixos_rebuild, so a
#     silent activation failure or OOM-kill is still caught.
# Falls back to run_nixos_rebuild automatically if `nh` isn't on PATH yet
# (e.g. a freshly imaged host that hasn't had its first home-manager
# activation, which is what installs `nh` via programs.nh).
run_nh_os() {
  local action="$1"
  shift

  if ! command -v nh >/dev/null 2>&1; then
    warn "'nh' not found on PATH — falling back to: sudo nixos-rebuild ${action} --flake ${NIXOS_DIR}#${FLAKE_ATTR}"
    run_nixos_rebuild "$action" --flake "${NIXOS_DIR}#${FLAKE_ATTR}" "$@"
    return $?
  fi
  ...
  local logfile
  logfile="$(mktemp /tmp/nh-os-XXXXXX.log)"

  local -a nh_args=(os "$action" "$NIXOS_DIR" -H "$FLAKE_ATTR")
  if [ "$#" -gt 0 ]; then
    nh_args+=("$@")
  fi
  if [ "${#extra_flags[@]}" -gt 0 ]; then
  ...
  log "Running: nh ${nh_args[*]}"
  if [ "${#extra_flags[@]}" -gt 0 ]; then
    ok "Host-specific flags (forwarded to nix build): ${C_BOLD}${extra_flags[*]}${C_RESET}"
  fi
  echo -e "${C_OVERLAY}Full log: ${logfile}${C_RESET}"

  local rc=0
  set +e
  nh "${nh_args[@]}" > >(tee "$logfile") 2>&1
  rc=$?
  set -e
  ...
  if [ "$rc" -ne 0 ]; then
    err "nh os ${action} failed with exit code ${rc}."
    err "Last lines of ${logfile}:"
    tail -30 "$logfile" >&2 || true
    return "$rc"
  fi
...
rollback_system() {
  # nh has no rollback subcommand — stays on nixos-rebuild directly.
...
clean_cache() {
  ...
  *)
  ...
  if confirm "Run 'nh os boot' to update the boot menu now?"; then
    ...
    warn "Bootloader NOT updated — the boot menu may keep showing removed generations until you run 'nh os boot' (or 'sudo nixos-rebuild boot') manually."
  fi
...
build_home_macbook() {
  ...
  # through run_nixos_rebuild/run_nh_os (those are `nh os` / `nixos-rebuild`,
  # both NixOS-system-only — the home-manager equivalent would be `nh home`,
  # not used here). Same git flow as build_flake (commit if dirty ->
  # checkout -> pull -> apply -> push), just without a NixOS host attr or
  # nixos-rebuild in the middle.
  require_dir
  cd "$NIXOS_DIR"
